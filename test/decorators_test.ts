...
import {Initial, StateDecorator, StateMachineDecorator} from '../src/decorators';

type MachineState = IStateDeclaration<Machine>;

@StateMachineDecorator(['mainState'])
class Machine extends StateMachine {
  @Initial text: string = 'do';
  @Initial alert: Record<string, any> = {
    text: 'alert',
    visible: false
  };

  @StateDecorator(StateMachine.INITIAL, ['requestState'])
  mainState: MachineState = {};

  @StateDecorator('mainState', ['successState', 'errorState'])
  requestState: MachineState = {
    text: 'request'
  };

  // Base state for extending only
  @StateDecorator('mainState', [])
  ...
  @StateDecorator('doneState', ['mainState'])
